...
# of the bar...
def waterfall(shap_values, max_display=10, show=True,fontsizeadd=0,equal_color="#999999",draw_expvalue=False):
    ...
    neg_low = []
    neg_high = []
    loc = base_values + values.sum() if draw_expvalue else values.sum()#累计值
    ...
    for i in range(num_individual):
        ...
        if sval >= 0:
            ...
            pos_lefts.append(loc)#post_lefts的loc自减
        else:
    ...
    # add a last grouped feature to represent the impact of all the features we didn't show,基本不需要
    if num_features < len(values):
    ...
    plt.yticks(ytick_pos, yticklabels[:-1] + [l.split('=')[-1] for l in yticklabels[:-1]], fontsize=13+fontsizeadd)

    # put horizontal lines for each feature row
    for i in range(num_features):
        plt.axhline(i, color="#cccccc", lw=0.5, dashes=(1, 5), zorder=-1)
    if draw_expvalue:
        # mark the prior expected value and the model prediction,Ef(x)和f(x)参考线
    ...
    ax.tick_params(labelsize=13+fontsizeadd)
    #plt.xlabel("\nModel output", fontsize=12)
    if draw_expvalue:
        # draw the E[f(X)] tick mark,Ef(x)label
        xmin, xmax = ax.get_xlim()
        ax2 = ax.twiny()
        ax2.set_xlim(xmin, xmax)
        ax2.set_xticks([base_values, base_values+1e-8])  # The 1e-8 is so matplotlib 3.3 doesn't try and collapse the ticks
        ax2.set_xticklabels(["\n$E[f(X)]$", "\n$ = "+format_value(base_values, "%0.03f")+"$"], fontsize=12+fontsizeadd, ha="left")
        ax2.spines['right'].set_visible(False)
        ax2.spines['top'].set_visible(False)
        ax2.spines['left'].set_visible(False)

        # draw the f(x) tick mark,f(x)label
        ax3 = ax2.twiny()
        ax3.set_xlim(xmin, xmax)
        # The 1e-8 is so matplotlib 3.3 doesn't try and collapse the ticks
        ax3.set_xticks([base_values + values.sum(), base_values + values.sum() + 1e-8])
        ax3.set_xticklabels(["$f(x)$", "$ = "+format_value(fx, "%0.03f")+"$"], fontsize=12+fontsizeadd, ha="left")
        #调整f(x)的位置
        ...
        tick_labels[1].set_color(equal_color)
        ax3.spines['right'].set_visible(False)
        ax3.spines['top'].set_visible(False)
        ax3.spines['left'].set_visible(False)

        # 调整E[f(X)] = x.xx label的位置
    ...
    for i in range(num_features):
        tick_labels[i].set_color(equal_color)##999999是灰色，#800080是紫色
...
def waterfall_legacy(expected_value, shap_values=None, features=None, feature_names=None, max_display=10, show=True,fontsizeadd=0):
    ...
    plt.yticks(list(range(num_features))*2, yticklabels[:-1] +
               [l.split('=')[-1] for l in yticklabels[:-1]], fontsize=13+fontsizeadd)
    ...
    ax2.set_xticklabels(["\n$E[f(X)]$", "\n$ = "+format_value(expected_value, "%0.03f")+"$"], fontsize=12+fontsizeadd, ha="left")
    ax2.spines['right'].set_visible(False)
    ax2.spines['top'].set_visible(False)
    ax2.spines['left'].set_visible(False)
